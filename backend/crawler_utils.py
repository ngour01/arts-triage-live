"""
Shared log-crawling helpers used by both the API (triage/discover)
and the standalone log_crawler.py script.
"""
import re
import logging
import requests
...
logger = logging.getLogger("art_triage.crawler_utils")
...
def get_full_psod_trace(summary_txt):
    """Extract all distinct PSOD / Panic / core traces from a testbedSummary HTML."""
    if not summary_txt:
        return []
...
def find_log_files(url, depth=0, max_depth=3, visited=None):
    """
    Recursively discover stateDump.json.txt files (not .gz) starting from *url*.
...
def discover_payloads(base_url, run_id, feature_name=None):
    """
    Smart discovery from *base_url*:
    - If the URL returns JSON with a 'result' key → single stateDump → 1 payload.
    - Otherwise treats it as an HTML directory listing and crawls recursively.

    Returns a list of payload dicts ready for _insert_attempt_logic / batch upload.
    Also fetches testbedSummary.html next to each stateDump for PSOD extraction.
    """
    txt = fetch(base_url)
    if not txt:
        return []
